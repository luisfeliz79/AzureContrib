...
Function Authenticate([Switch]$UseDeviceCode) {
...
function Restart-WebApps () {
    ...
    if ($Null -ne $Global:Auth -and $Global:Auth.IsValid() -eq $true -and $ForceAuthentication -eq $false) {
        ...
        $Global:Auth=Authenticate -UseDeviceCode:$UseDeviceCode
    } 

    # Get App List status
    $AppStatusResult=GetAppServiceWebAppInfo -ListOfIds $ListOfIds
    if ($AppStatusResult.count -gt 0) {
...
#Restart-WebApps -Path .\felizlabs-sample-1.txt
